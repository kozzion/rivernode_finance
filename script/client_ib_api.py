...
from rivernode_finance.client.ibapi import wrapper
from rivernode_finance.client.ibapi.client import EClient
from rivernode_finance.client.ibapi.contract import Contract, ContractDetails
from rivernode_finance.client.ibapi.common import OrderId, ListOfContractDescription, BarData,\
        HistogramDataList, TickerId
from rivernode_finance.client.ibapi.order import Order
from rivernode_finance.client.ibapi.order_state import OrderState
...
class IBApp(IBWrapper, IBClient):
    ...
    def __init__(self):
        ...
        self.dict_request = {}
        self.dict_request_end = {}
        self.dict_request_error = {}

    def error(self, request_id: TickerId, errorCode: int, errorString: str):
        """Overide EWrapper error method.
        """
    
        self.dict_request_error[request_id] = (errorCode, errorString)
        super().error(TickerId, errorCode, errorString)
        print(TickerId)
    ...
    def get_contract_details(self, symbol=None):
        ...
        if symbol not in self.saved_contract_details:
            ...
            for contract in self._contract_details:
                if contract.symbol == symbol and contract.currency == "USD" and contract.secType == "STK":
                    # NYSE stock
    ...
    def get_quote(self, symbol):
        ...
        self._historical_data = []
        self._historical_data_req_end = False
        self.reqHistoricalData(id_request, contract, end_date, duration, size, info, regular_trading_hours, 1, False, [])
        ...
        return float(quote.iloc[-1]['close_price'])
    ...
    def _req_historical_data(self, symbol, end_date="", duration="20 D",
        ...
        self._historical_data = []
        self._historical_data_req_end = False
        self.reqHistoricalData(2001, contract, end_date, duration, size, info, rth, 1, False, [])
    ...
    def contractDetails(self, request_id:int, contract_details:ContractDetails):
        ...
        if not request_id in self.dict_request:
            self.dict_request[request_id] = []
        self.dict_request[request_id] = contract_details

    # def contractDetailsEnd(self, request_id:int):
    #     print('contractDetailsEnd')
    #     self.dict_request_end[request_id] = True

    def await_contract_details(self, request_id:int):
        print('await_contract_details')
        while(True):
            if request_id in self.dict_request_end:
                return self.dict_request[request_id]
            if request_id in self.dict_request_error:
                error_code, error_message = dict_request_error[request_id]
            time.sleep(1)
            print('sleep')
            sys.stdout.flush()
...
if __name__ == "__main__":
    ...
    try:
        ...
        # app.reqFundamentalData(8001, ContractSamples.USStock(), "ReportsFinSummary");
        ...
        # app.reqContractDetails(id_request, contract)
        # contract_details = app.await_contract_details(id_request)
        ...
        # app.reqSecDefOptParams(id_request, underlyingSymbol, futFopExchange, underlyingSecType, underlyingConId)
        
        # app.await_cSecDefOptParams(id_request)

        contract_details = app.get_contract_details('FB')
        print(contract_details)
        # app.await_matching_symbols()
    # contractDetailsEnd(int reqId)
    # #         {
    #     app.g
        # list_quote = app.get_quotes(["AAPL"])
        ...
        exit()
        # app.cancelFundamentalData(8001)
