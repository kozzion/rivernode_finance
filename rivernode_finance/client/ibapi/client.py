...
from rivernode_finance.client.ibapi import (decoder, reader, comm)
from rivernode_finance.client.ibapi.connection import Connection
from rivernode_finance.client.ibapi.message import OUT
from rivernode_finance.client.ibapi.common import * # @UnusedWildImport
from rivernode_finance.client.ibapi.contract import Contract
from rivernode_finance.client.ibapi.order import Order
from rivernode_finance.client.ibapi.execution import ExecutionFilter
from rivernode_finance.client.ibapi.scanner import ScannerSubscription
from rivernode_finance.client.ibapi.comm import (make_field, make_field_handle_empty)
from rivernode_finance.client.ibapi.utils import (current_fn_name, BadMessage)
from rivernode_finance.client.ibapi.errors import * #@UnusedWildImport
from rivernode_finance.client.ibapi.server_versions import * # @UnusedWildImport
...
class EClient(object):
        ...
        try:
            ...
            while len(fields) != 2:
                ...
                if len(buf) > 0:
                    ...
                    print("here03")
                    print("size:%d msg:%s rest:%s|", size, msg, rest)
                    sys.stdout.flush()
                ...
                try:
                    try:
                        ...
                        if len(text) > MAX_MSG_LEN:
                            ...
                            print(text)
                            sys.stdout.flush()
                            self.wrapper.error(NO_VALID_ID, BAD_LENGTH.code(),"%s:%d:%s" % (BAD_LENGTH.msg(), len(text), text))
    ...
    def reqMatchingSymbols(self, reqId:int, pattern:str):
        ...
        msg = make_field(OUT.REQ_MATCHING_SYMBOLS) + make_field(reqId) + make_field(pattern)
